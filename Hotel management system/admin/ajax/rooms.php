          ...
          <tr class='align-middle'>
            ...
            <td>
              <button type='button' onclick='edit_details($row[room_id])' class='btn btn-primary shadow-none btn-sm' data-bs-toggle='modal' data-bs-target='#edit-room'>
                 ...
                 <button type='button' onclick='remove_room($row[room_id])' class='btn btn-danger shadow-none btn-sm'>
...
if(isset($_POST['edit_room']))
{
    ...
    $q1 = "UPDATE `rooms` SET `name`=?,`area`=?,`price`=?,`quantity`=?,`adult`=?,`children`=?,`description`=? WHERE `room_id`=?";
...
if(isset($_POST['add_image']))
 ...
 if(isset($_POST['remove_room']))
 {
    ...
    while($row = mysqli_fetch_assoc($res1)){
     ...
     $res5 = update("UPDATE `rooms` SET `removed`=? WHERE `room_id`=?",[1,$frm_data['room_id']],'ii');
