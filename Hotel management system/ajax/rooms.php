    ...
    while($room_data = mysqli_fetch_assoc($room_res))
    {
      ...
      if($chk_avail['checkin']!='' && $chk_avail['checkout']!='')
      {
        ...
        if(($room_data['quantity']-$tb_fetch['total_bookings'])==0){
           ...
           $fac_q = mysqli_query($con,"SELECT f.name,f.id FROM `facilites` f INNER JOIN `room_facilities` rfac ON f.id = rfac.facilities_id WHERE rfac.room_id = '$room_data[room_id]'");
        ...
        $features_data .="<span class='badge rounded-pill bg-light text-dark text-wrap me-1 mb-1'>
          ...
          if(!$settings_r['shutdown']){
            $login=0;
             ...
             $book_btn = "<button onclick='checkLoginToBook($login,$room_data[room_id])' class='btn btn-sm w-100  custom-bg shadow mb-2'>Book Now</button>";
          }
...
$output.="
  ...
  <h6 class='mb-4'>₹$room_data[price] per night</h6>
    $book_btn
  <a href='room_details.php?id=$room_data[room_id]' class='btn btn-sm w-100 btn-outline-dark  custom-bg shadow'>More details</a>
   </div>
   </div>
